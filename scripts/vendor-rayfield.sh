...
rayfield_sha=""
artifact_url=""
IFS=$'\t' read -r toolkit_version rayfield_version rayfield_url rayfield_sha artifact_url <<< "$metadata"
if [[ -z "$toolkit_version" || -z "$rayfield_version" || -z "$rayfield_url" || -z "$rayfield_sha" || -z "$artifact_url" ]]; then
...
download="$(mktemp "${destination}.XXXXXX.tmp")"
cleanup() {
    rm -f "$download"
}
trap cleanup EXIT
...
if ! grep -Fq "Rayfield Gen2 v${rayfield_version}" "$download"; then
    printf 'Downloaded Rayfield does not identify itself as version %s.\n' "$rayfield_version" >&2
    exit 1
fi
